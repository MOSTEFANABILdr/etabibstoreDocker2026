...
from .settings import (
    ...
    SEGMENTED_CODES,
    SEGMENT_LENGTH,
    SEGMENT_SEPARATOR, COUPON_TARGETS,
)
...
class CouponManager(models.Manager):
    def create_coupon(self, type, value, users=[], valid_until=None, prefix="", campaign=None, user_limit=None, target=None):
        coupon = self.create(
            value=value,
    ...
    def create_coupons(self, quantity, type, value, valid_until=None, prefix="", campaign=None, target=None):
        coupons = []
        for i in range(quantity):
            coupons.append(self.create_coupon(type, value, None, valid_until, prefix, campaign, target))
        return coupons
...
class Coupon(models.Model):
    ...
    target = models.CharField(_("Target"), max_length=2, choices=COUPON_TARGETS, default="1")
    ...
    def renderValue(self):
        if self.type == "percentage":
            return _("Réduction") + " {}%".format(self.value)
        if self.type == "monetary":
            return _("Réduction") + " {} DA".format(self.value)
        if self.type == "sponsorship":
            return _("Offre sponsorisée")
